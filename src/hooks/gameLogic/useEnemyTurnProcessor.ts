...
export const useTurnProcessor = (
  ...
  const processCollectorDroneActions = useCallback(async () => {
    setGameState((prev) => ({ ...prev, isProcessingCollectorDroneTurn: true, gamePhase: 'Animating' }))

    let currentDrones = [...gameState.placedCollectorDrones]
    let currentDataChips = [...gameState.dataChips]
    let currentSpareBatteriesOnMap = [...gameState.spareBatteriesOnMap]
    let currentSpareBatteriesCount = gameState.spareBatteriesCount
    let expGainedByDrones = 0

    const allOccupiedHexIdsForDrones = new Set<string>()
    if (gameState.player) allOccupiedHexIdsForDrones.add(axialToId(gameState.player.q, gameState.player.r))
    gameState.enemies.forEach((e) => allOccupiedHexIdsForDrones.add(axialToId(e.q, e.r)))
    gameState.placedBarricades.forEach((b) => allOccupiedHexIdsForDrones.add(axialToId(b.q, b.r)))

    for (let i = 0; i < currentDrones.length; i++) {
      let drone = { ...currentDrones[i] }

      drone.turnsLeft -= 1
      if (drone.turnsLeft <= 0) {
        addMessage('MESSAGE_COLLECTOR_DRONE_LIFESPAN_EXPIRED', { droneId: drone.id.substring(0, 4) })
        currentDrones.splice(i, 1)
        i--
        continue
      }
      ...
      const allItems = [...currentDataChips, ...currentSpareBatteriesOnMap]
      ...
      if (drone.targetItemId && closestItem) {
        const neighbors = getNeighbors(drone).filter((n) => {
          ...
          return (
            gameState.hexes.has(hexId) &&
            !gameState.hexes.get(hexId)?.isWall &&
            !allOccupiedHexIdsForDrones.has(hexId) &&
            !currentDrones.some((cd) => cd.id !== drone.id && cd.q === n.q && cd.r === n.r)
          )
        })
      ...
      const dataChipCollectedIndex = currentDataChips.findIndex((dc) => dc.q === drone.q && dc.r === drone.r)
      if (dataChipCollectedIndex !== -1) {
        const collectedChip = currentDataChips.splice(dataChipCollectedIndex, 1)[0]
      ...
      const batteryCollectedIndex = currentSpareBatteriesOnMap.findIndex((sb) => sb.q === drone.q && sb.r === drone.r)
      if (batteryCollectedIndex !== -1) {
        const collectedBattery = currentSpareBatteriesOnMap.splice(batteryCollectedIndex, 1)[0]
        currentSpareBatteriesCount += collectedBattery.count
        addMessage('MESSAGE_COLLECTOR_DRONE_COLLECTED_ITEM', {
          droneId: drone.id.substring(0, 4),
      ...
      currentDrones[i] = drone
    }

    setGameState((prev) => ({
      ...prev,
      placedCollectorDrones: currentDrones,
      dataChips: currentDataChips,
      spareBatteriesOnMap: currentSpareBatteriesOnMap,
      spareBatteriesCount: currentSpareBatteriesCount,
    }))

    if (expGainedByDrones > 0) {
      gainExperience(expGainedByDrones)
    }

    await new Promise((resolve) => setTimeout(resolve, ANIMATION_DURATION_MS * Math.max(1, currentDrones.length * 0.5)))
    setGameState((prev) => ({ ...prev, isProcessingCollectorDroneTurn: false, gamePhase: 'EnemyTurn' }))
  }, [gameState, setGameState, addMessage, t, gainExperience])

  const processEnemyTurns = useCallback(async () => {
    const initialGS = { ...gameState }

    const enemiesAtTurnStart = initialGS.enemies
    const playerAtTurnStartFromInitialGS = initialGS.player
    const trapsAtTurnStartSnapshot = initialGS.placedEmpTraps
    const barricadesAtTurnStartSnapshot = initialGS.placedBarricades
    const chargeFieldsAtTurnStartSnapshot = initialGS.placedChargeFields
    const collectorDronesAtTurnStartSnapshot = initialGS.placedCollectorDrones
    const currentHexes = initialGS.hexes
    const currentMapRad = initialGS.mapRadius
    const playerActiveBuffsAtTurnStart = initialGS.activeBuffs // Snapshot of buffs at START of enemy turn processing
    const currentArmamentLevels = initialGS.armamentLevels
    const currentMaxEnergy = initialGS.maxEnergy
    const currentBonusRecovery = initialGS.bonusEnergyRecoveryPerTurn
    const initialCurrentEnergyValue = initialGS.currentEnergy
    const initialSpareBatteriesCount = initialGS.spareBatteriesCount
    const currentMode = initialGS.gameMode
    const currentStageDefAtTurnStart = initialGS.currentStage
    const waveIdxAtTurnStart = initialGS.currentWaveIndex
    const pendingWaveSpawnsFromPrevTurn = initialGS.pendingWaveSpawns
    const currentTurnNum = initialGS.turnNumber
    const initialWaveTriggerTurn = initialGS.waveTriggerTurn
    const initialDataChips = initialGS.dataChips
    const initialSpareBatteries = initialGS.spareBatteriesOnMap
    const initialScore = initialGS.score
    const initialPlayerLevel = initialGS.playerLevel
    const initialCurrentExp = initialGS.currentExp
    const initialExpToNextLevel = initialGS.expToNextLevel
    const initialUpgradePoints = initialGS.upgradePoints
    const initialComboCount = initialGS.comboCount
    const initialCurrentComboBaseScore = initialGS.currentComboBaseScore
    const initialIsUpgradeSelection = initialGS.isUpgradeSelection
    const initialEscapePortalPosition = initialGS.escapePortalPosition
    ...
    if (chargeFieldsAtTurnStartSnapshot.length > 0) {
      ...
      if (strongestRecoveryAmount > 0 && initialGS.currentEnergy < currentMaxEnergy) {
        energyRecoveredFromChargeField = Math.min(strongestRecoveryAmount, currentMaxEnergy - initialGS.currentEnergy)
    ...
    let enemiesWithCalculatedFirstMove = enemiesAtTurnStart.map((enemy) => {
      ...
      const neighbors = getNeighbors(updatedEnemy).filter((n) => {
        const hexId = axialToId(n.q, n.r)
        const hex = currentHexes.get(hexId)
      ...
      if (neighbors.length > 0) {
        ...
        if (playerInSight && playerActiveBuffsAtTurnStart.stealth.isActive) {
          ...
          if (Math.random() > stealthStats.avoidanceChance / 100) {
            addLog({
              type: 'INFO',
              message: t('MESSAGE_HOSTILE_DETECTS_STEALTHED_PLAYER', { q: updatedEnemy.q, r: updatedEnemy.r }),
            })
          } else {
            playerInSight = false
            ...
            addLog({
              type: 'INFO',
              message: t('MESSAGE_HOSTILE_FAILS_TO_DETECT_STEALTHED', { q: updatedEnemy.q, r: updatedEnemy.r }),
            })
          }
        }
        if (playerInSight) {
    ...
    resolvedEnemyStatesAfterFirstMove = enemiesAtTurnStart.map((originalEnemy) => {
      ...
      if (finalOccupiedByMoversThisTurn.has(targetHexId)) {
        if (
          ...
          // Using t() for the log message to ensure it's a string, addressing potential type issue
          addLog({
            type: 'INFO',
            message: t('MESSAGE_HOSTILE_PATH_BLOCKED', {
              level: originalEnemy.level,
              q: originalEnemy.q,
              r: originalEnemy.r,
            }),
      ...
      if (Math.random() < DATA_CHIP_SPAWN_PROBABILITY) {
        const availableHexes = Array.from(currentHexes.values()).filter(
          (h) =>
            ...
            !spareBatteriesAfterSpawning.some((p) => p.q === h.q && p.r === h.r)
        )
        if (availableHexes.length > 0) {
          const spawnHex = availableHexes[Math.floor(Math.random() * availableHexes.length)]
          const existingChipIndex = dataChipsAfterSpawning.findIndex((dc) => dc.q === spawnHex.q && dc.r === spawnHex.r)
          if (existingChipIndex !== -1) {
            const updatedChip = {
              ...dataChipsAfterSpawning[existingChipIndex],
              count: dataChipsAfterSpawning[existingChipIndex].count + 1,
            }
            dataChipsAfterSpawning = dataChipsAfterSpawning.map((chip, index) =>
              index === existingChipIndex ? updatedChip : chip
            )
            addLog({
              type: 'INFO',
              message: `Endless: Data Chip STACKED at (${spawnHex.q},${spawnHex.r}). New Stack: ${updatedChip.count}`,
            })
          } else {
            const newChip = {
              id: nanoid(),
            ...
            addLog({
              type: 'INFO',
              message: `Endless: Data Chip SPAWNED at (${spawnHex.q},${spawnHex.r}). New Stack: ${newChip.count}`,
            })
          }
          addMessage('MESSAGE_ITEM_DETECTED_ON_MAP', { itemName: t(DATA_CHIP_CONFIG.nameKey) })
        }
      }
      if (Math.random() < SPARE_BATTERY_SPAWN_PROBABILITY) {
        const availableHexes = Array.from(currentHexes.values()).filter(
          (h) =>
            ...
            !occupiedHexesForSpawning.has(axialToId(h.q, h.r)) &&
            !dataChipsAfterSpawning.some((p) => p.q === h.q && p.r === h.r)
        )
        if (availableHexes.length > 0) {
          const spawnHex = availableHexes[Math.floor(Math.random() * availableHexes.length)]
          const existingBatteryIndex = spareBatteriesAfterSpawning.findIndex(
            (sb) => sb.q === spawnHex.q && sb.r === sb.r
          )
          if (existingBatteryIndex !== -1) {
            const updatedBattery = {
              ...spareBatteriesAfterSpawning[existingBatteryIndex],
              count: spareBatteriesAfterSpawning[existingBatteryIndex].count + 1,
            }
            spareBatteriesAfterSpawning = spareBatteriesAfterSpawning.map((battery, index) =>
              index === existingBatteryIndex ? updatedBattery : battery
            )
            addLog({
              type: 'INFO',
              message: `Endless: Spare Battery STACKED at (${spawnHex.q},${spawnHex.r}). New Stack: ${updatedBattery.count}`,
            })
          } else {
            ...
            addLog({
              type: 'INFO',
              message: `Endless: Spare Battery SPAWNED at (${spawnHex.q},${spawnHex.r}). New Stack: ${newBattery.count}`,
            })
          }
          // Ensuring itemName is a string for addMessage params
    ...
    if (currentMode === 'endless') {
      const availableHexesForEndlessSpawn = Array.from(currentHexes.values()).filter(
        (h) => h && !h.isWall && !occupiedHexesForSpawning.has(axialToId(h.q, h.r))
      )
      const safeHexesFar = availableHexesForEndlessSpawn.filter(
        (h) => axialDistance(h, tempPlayerState) >= ENEMY_MIN_SPAWN_DISTANCE_FROM_PLAYER
      )
      ...
      if (safeHexesFar.length > 0) {
        ...
        if (didSpawnEndless && currentTotalEnemyCount < currentHexes.size * 0.5) {
          ...
          if (enemyTypeAndLevel) {
            ...
            const candidates = safeHexesFar.map((h_1) => ({
              hex: h_1,
